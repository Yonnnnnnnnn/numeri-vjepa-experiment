...
- **CountVid Device Fix**: Konversi `torch.device` ke `string` pada pemanggilan `BatchEncoding.to()` di `groundingdino.py` untuk mencegah `TypeError` pada HuggingFace tokenizer.
