...
- **V4.2 Prompt Forwarding (Anti-Intent Collapse)**: The legacy visualizer (`engine_v2.py`) previously ignored the user's `--prompt` argument and used `identify_intent()` which forces a single-word answer (causing "Baked beans, Amoy, Custard..." → `"cans"`). Fixed by: (1) forwarding the prompt parameter through `main.py` → `run_v2_visualizer()`, (2) using `identify_foveated_intents()` when a prompt is provided, which supports multi-SKU comma-separated discovery, and (3) falling back to the original single-word discovery only when no prompt is given.
